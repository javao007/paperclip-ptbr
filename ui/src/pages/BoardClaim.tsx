...
export function BoardClaimPage() {
  ...
  if (!token || !code) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">URL de reivindicação de diretoria inválida.</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {statusQuery.error instanceof Error ? statusQuery.error.message : "Desafio inválido ou expirado."}
          </p>
        </div>
      </div>
    );
  ...
  if (status.status === "claimed") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">Propriedade da diretoria reivindicada</h1>
          <p className="mt-2 text-sm text-muted-foreground">
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Entre ou crie uma conta, depois retorne a esta página para reivindicar a propriedade da Diretoria.
          </p>
          <Button asChild className="mt-4">
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">Reivindicar propriedade da Diretoria</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Isso promoverá seu usuário a administrador de instância e migrará o acesso de propriedade da empresa do modo local confiável.
        </p>

        {claimMutation.error && (
          <p className="mt-3 text-sm text-destructive">
            {claimMutation.error instanceof Error ? claimMutation.error.message : "Falha ao reivindicar propriedade da diretoria"}
          </p>
        )}
