  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        <CardTitle className="text-base">Livro financeiro</CardTitle>
        <CardDescription>
          Cobranças a nível de conta que não correspondem a uma única solicitação de inferência.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} evento${eventCount === 1 ? "" : "s"} total no período`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label="Créditos"
          value={formatCents(creditCents)}
          subtitle="Reembolsos, compensações e devoluções de crédito"
          icon={ArrowDownLeft}
        />
        <MetricTile
          label="Net"
          value={formatCents(netCents)}
          subtitle="Débito menos crédito para o período selecionado"
          icon={ReceiptText}
        />
        <MetricTile
          label="Estimado"
          value={formatCents(estimatedDebitCents)}
          subtitle="Débitos estimados que ainda não são definitivos na fatura"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Selecione uma empresa para visualizar custos." />;
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  Gastos com inferência, taxas da plataforma, créditos e janelas de cota ao vivo.
                </p>
            </div>
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} agentes pausados · ${budgetData?.pausedProjectCount ?? 0} projetos pausados`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} of ${formatCents(spendData.summary.budgetCents)}`
                    : "Nenhum limite mensal configurado"
              }
              icon={Coins}
            />
            <MetricTile
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="billers">Cobradores</TabsTrigger>
          <TabsTrigger value="finance">Financeiro</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">Selecione uma data de início e fim para carregar os dados.</p>
          ) : showOverviewLoading ? (
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    <CardTitle className="text-base">Livro de inferência</CardTitle>
                    <CardDescription>
                      Gastos de inferência por solicitação no período selecionado.
                    </CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `Orçamento ${formatCents(spendData.summary.budgetCents)}`
                            : "Orçamento ilimitado"}
                        </div>
                      </div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% do orçamento mensal consumido neste período.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>O que cada agente consumiu no período selecionado.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">Nenhum evento de custo ainda.</p>
                    ) : (
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>Custos de execução atribuídos através de tarefas vinculadas a projetos.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Nenhum custo de execução atribuído a projeto ainda.</p>
                      ) : (
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Painel de controle de orçamento</CardTitle>
                  <CardDescription>
                    Limites rígidos de gastos para agentes e projetos. A cota de assinatura do provedor permanece separada e aparece em Provedores.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Violações de limite suave ou rígido em aberto"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Aprovações pendentes"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Aprovações de substituição de orçamento aguardando ação do conselho"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                  ...
                  <MetricTile
                    ...
                    subtitle="Execução de projetos bloqueada por orçamento"
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">Incidentes ativos</h2>
                    <p className="text-sm text-muted-foreground">
                      Resolva paradas aqui aumentando o orçamento ou mantendo explicitamente o escopo pausado.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Política mensal para toda a empresa."
                            : scopeType === "agent"
                              ? "Políticas de gastos mensais recorrentes para agentes individuais."
                              : "Políticas de gastos vitalícios para projetos vinculados a execução."}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Nenhuma política de orçamento ainda. Defina orçamentos de agentes e projetos em suas páginas de detalhe, ou use o controle de orçamento mensal existente da empresa.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Nenhum evento de custo neste período.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Nenhum evento cobrável neste período.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Por cobrador</CardTitle>
                      <CardDescription>Eventos financeiros a nível de conta agrupados por quem cobrou ou creditou.</CardDescription>
                    </CardHeader>
                    <CardContent className="grid gap-4 px-5 pb-5 pt-2 md:grid-cols-2">
                      {(financeData?.byBiller.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Nenhum evento financeiro ainda.</p>
                      ) : (
