...
import { issueStatusLabels, priorityLabels, t } from "../lib/translations";
...
function statusLabel(status: string): string {
  return issueStatusLabels[status] ?? priorityLabels[status] ?? status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? `Filtros: ${activeFilterCount}` : "Filtrar"}</span>
                {activeFilterCount > 0 && (
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="Nenhuma tarefa corresponde aos filtros ou pesquisa atuais."
          action="Criar Tarefa"
