...
export function FinanceTimelineCard({
  rows,
  emptyMessage = "Nenhum evento financeiro neste período.",
}: FinanceTimelineCardProps) {
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">Eventos financeiros recentes</CardTitle>
        <CardDescription>Recargas, taxas, créditos, compromissos e outras cobranças não vinculadas a requisições.</CardDescription>
      </CardHeader>
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>fatura {row.externalInvoiceId}</div> : null}
                      {row.region ? <div>região {row.region}</div> : null}
