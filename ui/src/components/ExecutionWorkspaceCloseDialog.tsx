...
export function ExecutionWorkspaceCloseDialog({
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? "Fechamento da área de trabalho tentado novamente" : "Área de trabalho fechada",
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
      ...
      <DialogContent className="max-h-[85vh] overflow-x-hidden overflow-y-auto p-4 sm:max-w-2xl sm:p-6 [&>*]:min-w-0">
        <DialogHeader>
          ...
          <DialogDescription className="break-words text-xs sm:text-sm">
            Arquivar <span className="font-medium text-foreground">{workspaceName}</span> e limpar quaisquer artefatos
            pertencentes à área de trabalho. O Paperclip mantém o registro da área de trabalho e o histórico de tarefas, mas a remove das visualizações de áreas de trabalho ativas.
          </DialogDescription>
        </DialogHeader>
        ...
        {readinessQuery.isLoading ? (
          ...
          <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-3 py-2.5 text-xs sm:px-4 sm:py-3 sm:text-sm text-destructive">
            {readinessQuery.error instanceof Error ? readinessQuery.error.message : "Falha ao verificar prontidão de fechamento."}
          </div>
        ) : readiness ? (
          <div className="min-w-0 space-y-3 sm:space-y-4">
            <div className={`rounded-xl border px-3 py-2.5 text-xs sm:px-4 sm:py-3 sm:text-sm ${readinessTone(readiness.state)}`}>
              ...
              <div className="mt-1 text-xs opacity-80">
                {readiness.isSharedWorkspace
                  ? "Esta é uma sessão de área de trabalho compartilhada. Arquivá-la remove este registro de sessão, mas mantém a área de trabalho do projeto subjacente."
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ...
                    : readiness.isProjectPrimaryWorkspace
                      ? "Esta área de trabalho de execução atualmente aponta para o caminho da área de trabalho principal do projeto."
            ...
            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-xs font-medium sm:text-sm">Problemas bloqueantes</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-xs font-medium sm:text-sm">Razões de bloqueio</h3>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-xs font-medium sm:text-sm">Outras tarefas vinculadas</h3>
                <div className="space-y-1.5 sm:space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-xs font-medium sm:text-sm">Serviços de runtime anexados</h3>
                <div className="space-y-1.5 sm:space-y-2">
            ...
            <section className="space-y-2">
              <h3 className="text-xs font-medium sm:text-sm">Ações de limpeza</h3>
              <div className="space-y-1.5 sm:space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5 text-xs sm:px-4 sm:py-3 sm:text-sm text-muted-foreground">
                A limpeza falhou anteriormente nesta área de trabalho. Tentar fechar novamente executará o fluxo de limpeza
                novamente e atualizará o status da área de trabalho se for bem-sucedido.
              </div>
            ) : null}
            ...
            {currentStatus === "archived" ? (
              <div className="rounded-xl border border-border bg-muted/20 px-3 py-2.5 text-xs sm:px-4 sm:py-3 sm:text-sm text-muted-foreground">
                Esta área de trabalho já está arquivada.
              </div>
            ) : null}

            {readiness.git?.repoRoot ? (
              <div className="overflow-hidden break-words text-xs text-muted-foreground">
                Raiz do repositório: <span className="font-mono break-all">{readiness.git.repoRoot}</span>
                {readiness.git.workspacePath ? (
                  <>
                    {" · "}Caminho da área de trabalho: <span className="font-mono break-all">{readiness.git.workspacePath}</span>
                  </>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              Última verificação {formatDateTime(new Date())}
            </div>
          </div>
        ) : null}
