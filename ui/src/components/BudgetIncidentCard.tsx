        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
              {incident.scopeType} parada forçada
            </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              Gasto atingiu {formatCents(incident.amountObserved)} contra um limite de {formatCents(incident.amountLimit)}.
            </CardDescription>
          </div>
        ...
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? "A execução do projeto está pausada. Novos trabalhos neste projeto não iniciarão até que você resolva o incidente de orçamento."
              : "Este escopo está pausado. Novas execuções não iniciarão até que você resolva o incidente de orçamento."}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          ...
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? "Aplicando..." : "Aumentar orçamento e retomar"}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              O novo orçamento deve exceder o gasto observado atual.
            </p>
          ) : null}
        </div>
