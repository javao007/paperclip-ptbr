...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"]) {
  return windowKind === "lifetime" ? "Orçamento vitalício" : "Orçamento mensal UTC";
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Observado</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% do limite` : "Nenhum teto configurado"}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">Orçamento</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : "Desativado"}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          Alerta suave em {summary.warnPercent}%{summary.paused && summary.pauseReason ? ` · pausa por ${summary.pauseReason}` : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>Restante</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : "Ilimitado"}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? "A execução está pausada para este projeto até que o orçamento seja aumentado ou o incidente seja resolvido."
          : "Os heartbeats estão pausados para este escopo até que o orçamento seja aumentado ou o incidente seja resolvido."}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? "Salvando..." : summary.amount > 0 ? "Atualizar orçamento" : "Definir orçamento"}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? "Pausado" : summary.status === "warning" ? "Alerta" : summary.status === "hard_stop" ? "Parada forçada" : "Saudável"}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">Insira um valor em dólares válido e não negativo.</p>
        ) : null}
      </div>
    );
  }
